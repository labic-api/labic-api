...
export default function ArticleEdit() {
  ...
  useEffect(() => {
    const loadArticle = async () => {
      try {
        ...
        if (data.authors) {
          // Os autores vêm como string separada por vírgula da API
          setAuthors(data.authors.split(',').map(a => a.trim()).filter(Boolean));
        }
        
        setIsDraft(data.status === 'Rascunho');
      } catch (err) {
        setErrors({ submit: 'Erro ao carregar dados do artigo.' });
      } finally {
        setInitialLoading(false);
      }
    };
    loadArticle();
  }, [id]); // Estado para travar o formulário na requisição
  ...
  const handleSubmit = async (e, isDraft = false) => {
    ...
    try {
      ...
      await articlesService.update(id, articleData);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.formCard}>
        <h1 style={styles.title}>Edição de Artigo</h1>
        <p style={styles.subtitle}>Atualize os dados do artigo acadêmico.</p>
        ...
        <form onSubmit={(e) => handleSubmit(e, false)} style={styles.form} noValidate>
          ...
          <div style={styles.actions}>
            <button 
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Salvando...' : (isDraft ? 'Salvar Alterações' : 'Publicar Alterações')}
            </button>
            <button 
              type="button" 
