...
export default function ResearcherEdit() {
  ...
  useEffect(() => {
    const loadResearcher = async () => {
      try {
        const data = await researchersService.getById(id);
        setFormData({
          name: data.name || '',
          email: data.email || '',
          area: data.area || '',
        ...
        setErrors({ submit: 'Erro ao carregar dados do pesquisador.' });
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Para edição, photoFile só é enviado se tiver mudado, mas vamos seguir o mesmo fluxo de mock
      await researchersService.update(id, researcherData);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.formCard}>
        <h1 style={styles.title}>Edição de Pesquisador</h1>
        <p style={styles.subtitle}>Atualize os dados do pesquisador abaixo.</p>
